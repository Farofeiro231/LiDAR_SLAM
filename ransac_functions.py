...
def landmark_extraction(pointsToBeFitted, landmarkNumber):#, yList, innerFlag):
...
def check_ransac(keyFlags, pairInliers, pointsToBeFitted):#, innerFlag):
    ...
    while True:
        if keyFlags.get(True) and len(pointsToBeFitted) > 2:
            ...
            print("Landmark ID: {}; posicao (x, y): ({}, {}) ;valor de a: {}; valor de b: {}" .format(extractedLandmark.get_pos()[0],extractedLandmark.get_pos()[1] , extractedLandmark.get_id(), extractedLandmark.get_a(), extractedLandmark.get_b()))
            landmarkNumber += 1
